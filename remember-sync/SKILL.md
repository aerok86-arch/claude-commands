---
name: remember-sync
description: 리멤버(rememberapp.co.kr) 개인명함첩을 Notion "👥 People" DB(2026-07-14부터 유일한 연락처 DB — 옛 "연락처" DB는 삭제됨)에 동기화한다. 리멤버 웹은 이메일 첨부가 아니라 웹 화면에서만 xlsx를 내보내므로, Gmail로 새 export가 생겼는지 확인한 뒤 브라우저 자동화로 card.rememberapp.co.kr에서 파일을 직접 다운로드하고, 명함등록일 체크포인트 기준 신규 인력만(전화번호 중복 체크 포함) Notion에 반영한다. Use when the user asks to sync Remember business cards to Notion, or says "리멤버 동기화", "명함 동기화해줘", "리멤버 연락처 업데이트", "remember sync", "명함첩 노션에 반영해줘".
allowed-tools:
  - Bash
  - Read
...
aerok86@gmail.com에 리멤버(no-reply@rememberapp.co.kr)의 "[remember] 명함 파일 생성이 완료되었습니다." 메일이 오면, 그 안에서 안내하는 파일을 받아 `~/notion-obsidian-sync/remember_sync.py`로 Notion "👥 People" DB(Life Hub 내 유일한 관계 자산 DB — 옛 "연락처" DB는 2026-07-14 사용자가 삭제함)에 **명함등록일 기준 신규분만, 전화번호가 이미 People에 있으면 스킵**하며 반영한다.
...
- 체크포인트는 "명함등록일"(리멤버 원본 필드) 기준. 최초 실행 시(체크포인트 파일 없을 때만) People DB 전체를 스캔해 현재 최댓값을 로컬 상태 파일에 시드해둔다.
- People DB는 이미 4천명 이상이 등록된 curated 관계 자산 DB라, `--apply` 시 생성 직전에 People 전체 전화번호를 한 번 더 스캔해서 **동일 전화번호가 이미 있으면 새 페이지를 만들지 않고 스킵**한다(중복 인물 생성 방지). 체크포인트는 스킵된 건도 포함해 갱신된다.
